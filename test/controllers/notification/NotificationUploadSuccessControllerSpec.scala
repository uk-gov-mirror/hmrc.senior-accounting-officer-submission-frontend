...
import NotificationUploadSuccessControllerSpec.*
  ...
  "NotificationUploadSuccess Controller" - {
    ...
    "when UpscanService returns State.NoReference" - {
      "must return Redirect to Journey recovery" in {
        when(
          mockUpscanService.fileUploadState(meq(UploadJourney.Notification), any[UserAnswers], any[Option[String]])(
            using any()
          )
        ).thenReturn(
        ...
        running(application) {
          val request =
            FakeRequest(
              GET,
              notificationRoutes.NotificationUploadSuccessController.onPageLoad(Some(testFileReference)).url
            )
    ...
    "when no key is provided" - {
      "must return Redirect to Journey recovery" in {
        when(
          mockUpscanService.fileUploadState(meq(UploadJourney.Notification), any[UserAnswers], meq(None))(using any())
        ).thenReturn(
          Future.successful(State.NoReference)
        )
        ...
        running(application) {
          val request =
            FakeRequest(GET, notificationRoutes.NotificationUploadSuccessController.onPageLoad(None).url)
    ...
    def testFileUploadStateCausesUploadFormRedirect(inState: State): Unit = {
      ...
      when(
        mockUpscanService.fileUploadState(meq(UploadJourney.Notification), any[UserAnswers], any[Option[String]])(using
          any()
        )
      ).thenReturn(
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).get mustEqual notificationRoutes.NotificationUploadFormController.onPageLoad().url
    ...
    "when UpscanService returns State.DownloadFromUpscanFailed" - {
      "must redirect to notification upload form page" in {
        ...
        running(application) {
          ...
          status(result) mustEqual SEE_OTHER
          redirectLocation(result).value mustBe notificationRoutes.NotificationUploadFormController.onPageLoad().url
    ...
    "when UpscanService returns State.Result" - {
      "must redirect to upload table page" in {
        ...
        running(application) {
          ...
          status(result) mustEqual SEE_OTHER
          redirectLocation(result).get mustBe notificationRoutes.UploadTemplateTableController.onPageLoad().url
